...
import { 
  ArrowLeft, Plus, Search, Tag, Trash2, 
  Edit, Info, Check, Square, DollarSign, ListFilter, Users 
} from "lucide-react";
...
export default function ListDetailView({
  ...
  const [newItemCategory, setNewItemCategory] = useState<Category>(Category.Despensa);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 bg-slate-900 p-3 rounded-xl border border-slate-800">
        <div className="text-center">
          <span className="text-[10px] text-slate-500 uppercase">Comprados</span>
          <p className="text-lg font-bold text-emerald-400">
            {list.items.filter((i) => i.purchased).length} / {list.items.length}
          </p>
        </div>
        <div className="text-center border-l border-slate-800">
          <span className="text-[10px] text-slate-500 uppercase">Presupuesto Est.</span>
      ...
      {showAddForm ? (
        ...
        >
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold text-emerald-400">Agregar Producto</span>
            <button
              type="button"
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[9px] uppercase font-mono tracking-wider text-slate-400">Nombre del Producto</label>
            <input
              type="text"
              value={newItemName}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-[9px] uppercase font-mono tracking-wider text-slate-400">Cantidad</label>
              <input
                type="number"
                min="1"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[9px] uppercase font-mono tracking-wider text-slate-400">Precio Est ($)</label>
              <input
                type="number"
                step="0.01"
                min="0.1"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-[9px] uppercase font-mono tracking-wider text-slate-400">Categoría</label>
              <select
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[9px] uppercase font-mono tracking-wider text-slate-400">Notas u Observación</label>
              <input
                type="text"
                value={newItemNotes}
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`text-xs font-bold leading-tight ${
                        item.purchased ? "line-through text-slate-500" : "text-white"
                      }`}
                    >
                      {item.name}
                    </span>
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className={`text-[8px] px-1.5 py-0.2 border rounded-full font-mono uppercase ${getCategoryColor(item.category)}`}>
                        {item.category}
                      </span>
                      {item.quantity && (
