...
import { Store, ShoppingList, Product } from "../types";
import { Navigation, MapPin, Sliders, BellRing, RefreshCw, ShoppingCart, HelpCircle } from "lucide-react";
...
export default function MapView({
  ...
  const pendingProducts = lists.flatMap((l) => l.items.filter((i) => !i.purchased));

  const handleScanPlaces = () => {
    setIsScanning(true);
    onLogSimulatation("Consultando Google Places API para detectar supermercados y farmacias...");
    setTimeout(() => {
      setIsScanning(false);
      onLogSimulatation(
  ...
  const handleSimulateEntersGeofence = (store: Store) => {
    onLogSimulatation(`Gatillando simulación de geocerca: Usuario ingresa al radio de ${store.distance} mi de ${store.name}`);
    
    // Find matching items
    const relevantPending = pendingProducts.filter((p) =>
      store.availableProducts.some(
        (ap) => ap.productName.toLowerCase().includes(p.name.toLowerCase()) || 
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-[11px] text-slate-400 mt-1">
          Simulador de geofencing en segundo plano de FOREMIND. Detecta tiendas a menos de 2 millas.
        </p>
      </div>
      ...
      <div className="relative aspect-video w-full bg-slate-900 border border-slate-850 rounded-2xl overflow-hidden mb-4 flex items-center justify-center">
        ...
        <div
          ...
          style={{
            width: alertRadius === 0.5 ? "35%" : alertRadius === 1.0 ? "65%" : "95%",
            height: alertRadius === 0.5 ? "35%" : alertRadius === 1.0 ? "65%" : "95%",
          }}
        />
      </div>
      ...
      <div className="bg-slate-900 border border-slate-850 p-3 rounded-xl mb-4">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-bold text-slate-200">Radio de Geofencing Preferido</span>
          <span className="text-xs font-mono font-bold text-emerald-400">{alertRadius} Millas</span>
        </div>
        <div className="flex gap-2">
          {[0.5, 1.0, 2.0].map((r) => (
            <button
              ...
              onClick={() => {
                onSetAlertRadius(r);
                onLogSimulatation(`Radio de geofencing actualizado a: ${r} mi (~${Math.round(r * 1.62)} km)`);
              }}
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] text-slate-400">({store.distance} mi)</span>
                </div>
              ...
              <div className="flex gap-1">
                <button
                  ...
                  title={!distanceMatches ? "Fuera del rango de alarma configurado" : "Simular geofencing"}
                >
