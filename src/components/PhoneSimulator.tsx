...
export default function PhoneSimulator({
  ...
  const handleSelectList = (listId: string) => {
    ...
    triggerSysLog(
      `Abriendo lista: '${lists.find((l) => l.id === listId)?.name}'`,
    );
  };
  ...
  const handleAddItem = (newItem: Omit<Product, 'id' | 'purchased'>) => {
    ...
    const updated = lists.map((l) => {
      if (l.id === selectedListId) {
        ...
        const storeProd = storeMatch.availableProducts.find(
          (ap) =>
            ap.productName.toLowerCase().includes(newItem.name.toLowerCase()) ||
    ...
    saveLists(updated);
    triggerSysLog(
      `Sincronizado con Firestore: Creado producto '${newItem.name}'`,
    );
  };
  ...
  const handleAddList = (name: string, category: string) => {
    const newListObj: ShoppingList = {
      ...
      category,
      items: [],
      members: [{ email: profile.email, name: profile.name, role: 'admin' }],
    };
    saveLists([...lists, newListObj]);
    triggerSysLog(
      `Creado contenedor Firestore: Nueva lista '${name}' [${category}]`,
    );
  };
  ...
  const handleInviteMember = (
    ...
    saveLists(updated);
    triggerSysLog(
      `Invitación en tiempo real: Se unió '${name}' (${email}) con rango '${role}'`,
    );
  };
  ...
  const handleGeofencePushAndPrompt = (
    store: Store,
    matchedPending: Product[],
  ) => {
    if (matchedPending.length === 0) {
      triggerSysLog(
        `Detección de radio de ${store.name} aprobada, pero no tienes ningún producto de esta tienda pendiente.`,
      );
      alert(
        `Estás cerca de ${store.name}, pero de momento no tienes pendientes de tu lista para este supermercado.`,
      );
      return;
    }
    ...
    const itemPrice =
      store.availableProducts.find(
        (ap) =>
          ap.productName
            .toLowerCase()
            .includes(targetItem.name.toLowerCase()) ||
    ...
    const matchedList = lists.find((l) =>
      l.items.some((i) => i.id === targetItem.id),
    );
    ...
    triggerSysLog(
      `FCM Cloud Messaging: Enviado Push payload para geocerca comercial de '${store.name}'`,
    );
  };
  ...
  const handleAcceptPurchasePrompt = () => {
    ...
    const updated = lists.map((l) => {
      if (l.id === listId) {
        return {
          ...l,
          items: l.items.map((i) =>
            i.id === productId ? { ...i, purchased: true } : i,
          ),
        };
      }
      return l;
    ...
    saveLists(updated);
    triggerSysLog(
      `Compra confirmada. Sincronizado en Firestore: '${productName}' comprado en '${storeName}'.`,
    );

    setActivePurchasePrompt(null);
    alert(
      `¡Listo! Se marcó como comprado en la lista '${lists.find((l) => l.id === listId)?.name}'.`,
    );
  };
  ...
  return (
    <div className='flex flex-col items-center justify-center p-2 w-full max-w-sm mx-auto'>
      {/* Shell Selector */}
      <div
        id='device-shell-selector'
        className='flex items-center gap-2 mb-4 bg-slate-900/60 p-1 rounded-full border border-slate-800'
      >
        <button
          onClick={() => {
            setDeviceShell('ios');
      ...
      >
        ...
        {deviceShell === 'ios' ? (
          ...
          <div className='absolute top-2.5 left-1/2 -translate-x-1/2 w-28 h-6 bg-black rounded-2xl z-50 flex items-center justify-center px-1'>
            ...
            <span className='text-[8px] text-emerald-400 font-mono font-bold tracking-tighter'>
              FOREMIND GPS
            </span>
          </div>
        ) : (
        ...
        <div
          ...
          className='h-7 w-full bg-slate-950 flex justify-between items-center px-6 pt-1 text-[10px] text-slate-400 font-sans tracking-wide z-40 select-none'
        >
          <span>08:08 AM</span>
          <div className='flex items-center gap-1'>
            <span className='text-[8px] font-mono font-black uppercase text-emerald-500 tracking-tighter'>
              5G LTE
            </span>
        ...
        {activeNotification && (
          ...
          >
            ...
            <div className='flex-1 text-left'>
              ...
              <h5 className='text-[11px] font-black text-white mt-0.5 leading-tight'>
        ...
        {/* SCREEN CANVAS AREA */}
        <div
          id='app-viewport-canvas'
          className='flex-1 w-full bg-slate-950 overflow-hidden relative flex flex-col pt-0 pb-1.5'
        >
          ...
          {currentScreen === 'splash' && (
            <SplashView
              onDismiss={() => {
                ...
                triggerSysLog(
                  'Splash screen de FOREMIND completada. Solicitando credenciales...',
                );
              }}
            />
          )}
          ...
          {currentScreen === 'login' && (
            <LoginView
              onLoginSuccess={(email, name) => {
                ...
                triggerSysLog(
                  `Sesión de Firebase iniciada: Autenticado como ${name} (${email})`,
                );
              }}
            />
          )}
          ...
          {activePurchasePrompt && (
            <div
              id='alert-overlay-bg'
              className='absolute inset-0 bg-black/75 flex items-center justify-center p-4 z-40 animate-fadeIn'
            >
              <div
                id='alert-box'
              ...
              >
                ...
                <div>
                  ...
                  <h4 className='text-xs font-bold text-white mt-1 leading-normal'>
                    ...
                    {activePurchasePrompt.storeName} a $
                    {activePurchasePrompt.price}?
                  </h4>
                  <p className='text-[10px] text-slate-400 mt-1 leading-snug'>
                    Detectado a {activePurchasePrompt.distance} millas de tu
                    ubicación física.
                  </p>
                </div>
        ...
        {currentScreen !== 'splash' && currentScreen !== 'login' ? (
          <div
            ...
            className='h-[52px] bg-slate-950 border-t border-slate-900/80 w-full flex items-center justify-around text-slate-500 z-40 select-none pb-0.5'
          >
            <button
              onClick={() => {
                setCurrentScreen('lists');
